...
// Note: 'tv_movie' is explicitly included here to ensure it's treated as a movie for tracking/display.
$is_series = ! in_array( $details_data['type'], array( 'movie', 'short_film', 'doc_film', 'tv_movie' ) );
...
// --- Dynamic URL Definition (FIXED) ---
$my_tracker_base_url = trailingslashit( $permalink ) . 'my-shows';
if (!$is_series) {
    // If it's a movie (or tv_movie, short_film, doc_film), link to the movie tracker sub-page
    $my_tracker_url = trailingslashit( $my_tracker_base_url ) . 'movies';
} else {
    // If it's a series, link to the main series tracker page
    $my_tracker_url = $my_tracker_base_url;
}
...
<div class="tvm-details-header">
    ...
    <div class="tvm-details-actions">
        <!-- LINK NOW POINTS TO SERIES OR MOVIE TRACKER DYNAMICALLY -->
        <a href="<?php echo esc_url( $my_tracker_url ); ?>" class="tvm-button tvm-button-details"><?php esc_html_e( 'My Tracker', 'tvm-tracker' ); ?></a>
        <a href="<?php echo esc_url( $back_to_search_url ); ?>" class="tvm-button tvm-button-back"><?php esc_html_e( 'Back to Search', 'tvm-tracker' ); ?></a>
        ...
        <!-- Add/Remove Tracker Button (For Series and Movie Removal) -->
        <?php if ($is_series || $is_tracked): ?>
            <button type="button" 
                class="tvm-button <?php echo esc_attr( $button_class ); ?> tvm-toggle-show-btn" 
                data-title-id="<?php echo absint( $title_id ); ?>"
        ...
        <!-- Add Tracker Buttons (For Untracked Movies only) -->
        <?php if (!$is_series && !$is_tracked): ?>
            <button type="button" 
                class="tvm-button tvm-button-add tvm-toggle-show-btn" 
                data-title-id="<?php echo absint( $title_id ); ?>"
                data-title-name="<?php echo esc_attr( $details_data['title'] ); ?>"
                data-total-episodes="<?php echo absint( count( $episodes_data ) ); ?>"
                data-is-tracked="false"
                data-item-type="<?php echo esc_attr( $item_type ); ?>" 
                data-release-date="<?php echo esc_attr( $release_date ); ?>"
                data-is-movie-watched="false"
            >
                <?php esc_html_e('Add to Tracker (Want to See)', 'tvm-tracker'); ?>
            </button>
            <button type="button" 
                class="tvm-button tvm-button-watched tvm-toggle-show-btn" 
                data-title-id="<?php echo absint( $title_id ); ?>"
                data-title-name="<?php echo esc_attr( $details_data['title'] ); ?>"
                data-total-episodes="<?php echo absint( count( $episodes_data ) ); ?>"
                data-is-tracked="false"
                data-item-type="<?php echo esc_attr( $item_type ); ?>" 
                data-release-date="<?php echo esc_attr( $release_date ); ?>"
                data-is-movie-watched="true"
            >
                <?php esc_html_e('Add to Tracker (Seen It)', 'tvm-tracker'); ?>
            </button>
        <?php endif; ?>
    </div>
</div>
    ...
    <div class="tvm-info-column">
        ...
        <?php if (!$is_series) : 
            $release_date_display = ($release_date && $release_date !== '0000-00-00') 
                // FIX: Replaced date() with date_i18n() for WordPress compatibility.
