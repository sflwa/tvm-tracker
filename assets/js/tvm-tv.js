 ...
 * Version: 1.3.9 - Deep Linking & Aired Count Logic
 * Author: South Florida Web Advisors
 */
jQuery(function($) {
    const TVModule = {
        init: function() {
            // Check for direct bookmark link
            if (window.location.hash === '#tv-unwatched') {
                ...
                setTimeout(() => {
                    ...
                    $('.tvm-filter-btn[data-filter="released"]').addClass('active');
                    this.load();
                }, 100);
            }
            ...
            $(document).on('click', '.tvm-dropdown-nav', (e) => {
                ...
                let nextIndex = (direction === 'next') ? 
                    (currentIndex >= totalOptions - 1 ? 1 : currentIndex + 1) : 
                    (currentIndex <= 1 ? totalOptions - 1 : currentIndex - 1);
            ...
            $(document).on('click', '.tvm-delete-item', function(e) {
                ...
                if (confirm('Remove this series?')) {
                    $.post(tvm_app.ajax_url, { action: 'tvm_delete_item', post_id: id, nonce: tvm_app.nonce }, () => TVModule.load());
                }
            });
                ...
                if (res.success) {
                    ...
                    seasons.forEach((sNum, index) => {
                        ...
                        contentHtml += `<div id="tvm-season-group-${sNum}" class="tvm-season-content-group" style="display:${isActive ? 'flex' : 'none'}; flex-direction:column; gap:12px;">
                            <div style="display:flex; justify-content:flex-end; margin-bottom:10px;"><button class="tvm-mark-season-watched button button-primary button-small" style="font-size:10px; font-weight:700;">Mark Full Season Watched</button></div>`;
                        grouped[sNum].forEach(ep => {
                            const statusColor = ep.is_watched ? '#46b450' : '#ddd';
                            contentHtml += `<div class="tvm-episode-row" style="border-left-color: ${statusColor};">
                                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                                    <div style="flex:1;"><div style="font-weight:800; color:#1d2327; font-size:16px;">E${ep.number} - ${ep.title.replace(/^S\d+E\d+\s-\s/i, '')}</div><div style="font-size:12px; color:#999; margin-top:4px;">Air Date: ${ep.air_date}</div></div>
                                    <div style="display:flex; align-items:center; gap:15px;"><div class="tvm-episode-sources" style="display:flex; gap:6px;">${ep.is_future ? '<span style="font-size:10px; color:#2271b1; background:#e7f3ff; padding:4px 8px; border-radius:4px; font-weight:700;">Upcoming</span>' : this.renderSources(ep.sources)}</div>
                                    ${ep.is_future ? '<span class="dashicons dashicons-clock" style="color:#bbb; font-size:24px;"></span>' : `<span class="dashicons ${ep.is_watched ? 'dashicons-visibility' : 'dashicons-hidden'} tvm-ep-watch" data-id="${ep.id}" data-watched="${!ep.is_watched}" style="cursor:pointer; color:${statusColor}; font-size:24px;"></span>`}</div>
                                </div><div class="tvm-ep-overview" style="margin-top:12px; font-size:13px; line-height:1.5; color:#666;">${ep.overview || 'No description available.'}</div></div>`;
                        });
                        contentHtml += `</div>`;
                    });
